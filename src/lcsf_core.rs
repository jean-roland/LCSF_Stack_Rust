...
impl LcsfCore {
    ...
    pub fn add_protocol(&mut self, prot_id:u16, prot_desc:&'static LcsfProtDesc, prot_cb:ProtCallback) {
    ...
    pub fn receive_buff(&self, buff:&[u8]) -> bool {
        ...
        let raw_msg = match lcsf_transcoder::decode_buff(self.lcsf_mode, buff) {
            Err(err) => {
                ...
                if self.do_gen_err {
                    ...
                    let buff = lcsf_error::encode_error(self.lcsf_mode, LcsfEpLocEnum::DecodeError, err as u8);
                    (self.fn_send)(buff);
                }
                return false;
            },
        ...
        let (valid_msg, prot_id) = match lcsf_validator::validate_msg(&self.prot_desc_map, &raw_msg) {
            Err(err) => {
                ...
                if self.do_gen_err {
                    // Generate and send error
                    let buff = lcsf_error::encode_error(self.lcsf_mode, LcsfEpLocEnum::ValidationError, err as u8);
                    (self.fn_send)(buff);
                }
                return false;
            },
...
lazy_static! {
    static ref TEST_PROT_DESC:LcsfProtDesc = LcsfProtDesc {
        cmd_desc_arr: vec![
            (0x12, LcsfCmdDesc {att_desc_arr: Vec::new(),}),
        ]
    };
...
static mut CMD_IS_VALID:AtomicBool = AtomicBool::new(false);
...
fn test_receive_buff() {
    ...
    // Check value
    let is_valid:bool = unsafe {
        CMD_IS_VALID.load(Ordering::SeqCst)
    };
    assert!(is_valid);
}

static mut BUFF_IS_VALID:AtomicBool = AtomicBool::new(false);
...
fn test_send_cmd() {
    ...
    let is_valid:bool = unsafe {
        BUFF_IS_VALID.load(Ordering::SeqCst)
    };
    assert!(is_valid);
}

static mut ERR_IS_VALID:AtomicBool = AtomicBool::new(false);
...
fn test_error_processing() {
    ...
    // Check value
    let is_valid:bool = unsafe {
        ERR_IS_VALID.load(Ordering::SeqCst)
    };
    assert!(is_valid);
}

static mut BAD_DATA_IS_VALID:AtomicBool = AtomicBool::new(false);
...
fn test_error_encoding() {
    ...
    // Check value
    let is_valid:bool = unsafe {
        BAD_DATA_IS_VALID.load(Ordering::SeqCst)
    };
    assert!(is_valid);
}
}