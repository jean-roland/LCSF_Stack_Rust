...
pub enum LcsfRawAttPayload {
    Data(Vec<u8>), // Pointer to data
    SubattrArr(Vec<LcsfRawAtt>), // Pointer to lcsf sub attribute header list
}
...
pub struct LcsfRawAtt {
    ...
    pub att_id:u16, // Lcsf attribute id
    pub payload_size:u16, // Data size (bytes) or lcsf sub attribute number
...
pub struct LcsfRawMsg {
    ...
    pub att_arr:Vec<LcsfRawAtt>, // Pointer to the lcsf attribute header list
}
...
/// \param lcsf_mode parsing mode to use
/// \param buff_iter reference to the buffer iterator
fn fetch_att_header(lcsf_mode:LcsfModeEnum, buff_iter:&mut Iter<u8>) -> Option<LcsfRawAtt> {
    ...
    if lcsf_mode == LcsfModeEnum::Small {
        ...
        att.att_id = byte1 & !(1 << 7); // Mask the flag from the id
        // Byte 2: Payload size
        att.payload_size = *buff_iter.next()? as u16;
    } else {
        // Byte 1: Attribute id LSB
        att.att_id = *buff_iter.next()? as u16;
        ...
        att.att_id += (byte2 & !(1 << 7)) << 8; // Mask the flag from the id
...
fn test_fetch_att_header() {
    let mut att = TX_MSG.att_arr[0].clone();
    att.payload = LcsfRawAttPayload::Data(Vec::new());

    // Test error
    assert_eq!(None, fetch_att_header(LcsfModeEnum::Small, &mut [].iter()));
    // Test small
    let mut new_att = fetch_att_header(LcsfModeEnum::Small, &mut RX_MSG_SMALL[3..].iter()).unwrap();
    assert_eq!(new_att, att);
    // Test normal
    new_att = fetch_att_header(LcsfModeEnum::Normal, &mut RX_MSG_NORMAL[6..].iter()).unwrap();
    assert_eq!(new_att, att);
}
...
/// \param lcsf_mode parsing mode to use
/// \param buff_iter reference to the buffer iterator
fn decode_att_rec(lcsf_mode:LcsfModeEnum, buff_iter:&mut Iter<u8>) -> Result<LcsfRawAtt, LcsfDecodeErrorEnum> {
    // Decode current attribute header
    let mut att = match fetch_att_header(lcsf_mode, buff_iter) {
        None => return Err(LcsfDecodeErrorEnum::FormatErr),
        Some(att_header) => att_header,
    };
    // Test if attribute has data or sub-attributes
    if att.has_subatt {
        att.payload = LcsfRawAttPayload::SubattrArr(Vec::new());
        // Parse through the attribute array
        for _att_idx in 0..att.payload_size {
            // Decode sub-attribute
            let subatt = decode_att_rec(lcsf_mode, buff_iter)?;
            // Add sub-attribute
            if let LcsfRawAttPayload::SubattrArr(subatt_arr) = &mut att.payload {
                subatt_arr.push(subatt);
            };
        }
    } else {
...
fn test_decode_att_rec() {
    ...
    for att_idx in 0..TX_MSG.att_arr.len() {
        match decode_att_rec(LcsfModeEnum::Small, data_iter) {
            Ok(new_att) => assert_eq!(new_att, TX_MSG.att_arr[att_idx]),
...
pub fn decode_buff(lcsf_mode:LcsfModeEnum, buffer:&[u8]) -> Result<LcsfRawMsg, LcsfDecodeErrorEnum> {
    ...
    for _idx in 0..dec_msg.att_nb {
        let new_att = decode_att_rec(lcsf_mode, buff_iter)?;
        // Store attribute
        dec_msg.att_arr.push(new_att);
    }
...
fn test_decode_buff() {
    ...
    match decode_buff(LcsfModeEnum::Small, RX_MSG_SMALL) {
        Ok(new_msg) => assert_eq!(new_msg, *TX_MSG),
...
fn test_fill_msg_header() {
    // Test small
    assert_eq!(fill_msg_header(LcsfModeEnum::Small, &TX_MSG), vec![0xab, 0x12, 0x03]);
    // Test normal
    assert_eq!(fill_msg_header(LcsfModeEnum::Normal, &TX_MSG), vec![0xab, 0x00, 0x12, 0x00, 0x03, 0x00]);
}
...
fn fill_att_header(lcsf_mode:LcsfModeEnum, att:&LcsfRawAtt) -> Vec<u8> {
    let mut buffer:Vec<u8> = Vec::new();
...
fn test_fill_att_header() {
    // Test small
    assert_eq!(fill_att_header(LcsfModeEnum::Small, &TX_MSG.att_arr[0]), vec![0x55, 0x05]);
    assert_eq!(fill_att_header(LcsfModeEnum::Small, &TX_MSG.att_arr[1]), vec![0xff, 0x02]);
    // Test normal
    assert_eq!(fill_att_header(LcsfModeEnum::Normal, &TX_MSG.att_arr[0]), vec![0x55, 0x00, 0x05, 0x00]);
    assert_eq!(fill_att_header(LcsfModeEnum::Normal, &TX_MSG.att_arr[1]), vec![0x7f, 0x80, 0x02, 0x00]);
}
...
fn encode_att_rec(lcsf_mode:LcsfModeEnum, att:&LcsfRawAtt) -> Vec<u8> {
    let mut buffer:Vec<u8> = Vec::new();

    // Fill attribute header
    buffer.extend(fill_att_header(lcsf_mode, att));
    // Check payload type
    match &att.payload {
        ...
        LcsfRawAttPayload::SubattrArr(subatt_arr) => {
            // Parse sub-attribute array
            for sub_att in subatt_arr {
                // Encode sub-attribute in buffer
                buffer.extend(encode_att_rec(lcsf_mode, &sub_att));
            }
        }
    }
    return buffer;
...
fn test_encode_att_rec() {
    // Test small
    assert_eq!(encode_att_rec(LcsfModeEnum::Small, &TX_MSG.att_arr[1]), RX_MSG_SMALL[10..21]);
    // Test normal
    assert_eq!(encode_att_rec(LcsfModeEnum::Normal, &TX_MSG.att_arr[1]), RX_MSG_NORMAL[15..34]);
}
...
pub fn encode_buff(lcsf_mode:LcsfModeEnum, msg:&LcsfRawMsg) -> Vec<u8> {
    ...
    for att in &msg.att_arr {
        buffer.extend(encode_att_rec(lcsf_mode, &att));
    }
    return buffer;
}
...
fn test_encode_buff() {
    // Test small
    assert_eq!(encode_buff(LcsfModeEnum::Small, &TX_MSG), RX_MSG_SMALL);
    // Test normal
    assert_eq!(encode_buff(LcsfModeEnum::Normal, &TX_MSG), RX_MSG_NORMAL);
}
...
#[cfg(test)]
lazy_static! {
    static ref TX_MSG:LcsfRawMsg = LcsfRawMsg {
        prot_id: 0xab,
        cmd_id: 0x12,
        att_nb: 3,
        att_arr: vec![
            ...
            LcsfRawAtt {
                ...
                att_id: 0x7f,
                payload_size: 2,
                payload: LcsfRawAttPayload::SubattrArr(vec![
                    LcsfRawAtt {
                        has_subatt: false,
                        att_id: 0x30,
                        payload_size: 1,
