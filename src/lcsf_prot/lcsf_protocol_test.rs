...
fn cc2_get_data(att_arr: &[LcsfValidAtt]) -> CmdPayload {
    ...
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc2_payload.sa5 = String::from_utf8(data.clone()).unwrap();
    }
    // Retrieve data of optional attribute SA6
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
    ...
    // Retrieve data of optional attribute SA7
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
    ...
    // Retrieve data of optional attribute SA8
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
    ...
    // Retrieve data of optional attribute SA9
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
    ...
    // Retrieve data of optional attribute SA10
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
            cc2_payload.is_sa10_here = true;
            cc2_payload.sa10 = String::from_utf8(data.clone()).unwrap();
        }
    }
...
fn cc3_get_data(att_arr: &[LcsfValidAtt]) -> CmdPayload {
    ...
    // Retrieve data of attribute SA5
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        cc3_payload.sa5 = String::from_utf8(data.clone()).unwrap();
    }
    // Retrieve data of optional attribute SA6
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
    ...
    if let LcsfValidAttPayload::Data(data) = &att_iter.next().unwrap().payload {
        if *data != Vec::new() {
            ...
            cc3_payload.sa10 = String::from_utf8(data.clone()).unwrap();
        }
    }
...
fn cc5_get_data(att_arr: &[LcsfValidAtt]) -> CmdPayload {
    let mut cc5_payload = Cc5AttPayload {
        ...
        ca6_payload: Cc5AttCa6Payload {
            ...
            ca7_payload: Ca6AttCa7Payload {
                is_sa1_here: false,
                sa1: 0,
                ca8_payload: Ca7AttCa8Payload { sa4: Vec::new() },
            },
        },
    };
    // Get iterator
    ...
    if let LcsfValidAttPayload::SubattArr(subatt_arr) = &att_iter.next().unwrap().payload {
        ...
        // Retrieve data of optional sub-attribute SA3
        if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
            if *data != Vec::new() {
                cc5_payload.ca5_payload.is_sa3_here = true;
                cc5_payload.ca5_payload.sa3 =
                    u32::from_le_bytes(data.as_slice().try_into().unwrap());
            }
        }
    }
    // Retrieve data of optional attribute CA6
    if let LcsfValidAttPayload::SubattArr(subatt_arr) = &att_iter.next().unwrap().payload {
        if *subatt_arr != Vec::new() {
            cc5_payload.is_ca6_here = true;
            // Get iterator
            let subatt_iter = &mut subatt_arr.iter();
            // Retrieve data of optional sub-attribute SA1
            if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
                if *data != Vec::new() {
                    cc5_payload.ca6_payload.is_sa1_here = true;
                    cc5_payload.ca6_payload.sa1 =
                        u8::from_le_bytes(data.as_slice().try_into().unwrap());
                }
            }
            ...
            if let LcsfValidAttPayload::SubattArr(subatt_arr) = &subatt_iter.next().unwrap().payload
            {
                ...
                if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
                    if *data != Vec::new() {
                        ...
                        cc5_payload.ca6_payload.ca7_payload.sa1 =
                            u8::from_le_bytes(data.as_slice().try_into().unwrap());
                    }
                }
                // Retrieve data of sub-attribute CA8
                if let LcsfValidAttPayload::SubattArr(subatt_arr) =
                    &subatt_iter.next().unwrap().payload
                {
                    // Get iterator
                    let subatt_iter = &mut subatt_arr.iter();
    ...
    if let LcsfValidAttPayload::SubattArr(subatt_arr) = &att_iter.next().unwrap().payload {
        ...
        if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
            if *data != Vec::new() {
                ...
                cc6_payload.ca9_payload.sa3 =
                    u32::from_le_bytes(data.as_slice().try_into().unwrap());
            }
        }
    }
    // Retrieve data of optional attribute CA10
    if let LcsfValidAttPayload::SubattArr(subatt_arr) = &att_iter.next().unwrap().payload {
        if *subatt_arr != Vec::new() {
            ...
            if let LcsfValidAttPayload::SubattArr(subatt_arr) = &subatt_iter.next().unwrap().payload
            {
                ...
                if let LcsfValidAttPayload::Data(data) = &subatt_iter.next().unwrap().payload {
                    if *data != Vec::new() {
                        ...
                        cc6_payload.ca10_payload.ca11_payload.sa1 =
                            u8::from_le_bytes(data.as_slice().try_into().unwrap());
                    }
                }
...
fn cc1_fill_att(cmd_payload: &CmdPayload) -> Vec<LcsfValidAtt> {
    ...
    if let CmdPayload::Cc1Payload(payload) = &cmd_payload {
        ...
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::Data(payload.sa1.to_le_bytes().to_vec()),
        });
        // Fill data of attribute SA2
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::Data(payload.sa2.to_le_bytes().to_vec()),
        });
        // Fill data of attribute SA3
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::Data(payload.sa3.to_le_bytes().to_vec()),
        });
        // Fill data of attribute SA4
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::Data(payload.sa4.clone()),
        });
        // Fill data of attribute SA5
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::Data(payload.sa5.as_bytes().to_vec()),
        });
        // Fill data of attribute SA6
        if payload.is_sa6_here {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(payload.sa6.to_le_bytes().to_vec()),
            });
        } else {
            att_arr.push(LcsfValidAtt {
        ...
        if payload.is_sa7_here {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(payload.sa7.to_le_bytes().to_vec()),
            });
        } else {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(Vec::new()),
        ...
        if payload.is_sa8_here {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(payload.sa8.to_le_bytes().to_vec()),
            });
        } else {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(Vec::new()),
        ...
        if payload.is_sa9_here {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(payload.sa9.clone()),
            });
        } else {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(Vec::new()),
        ...
        if payload.is_sa10_here {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(payload.sa10.as_bytes().to_vec()),
            });
        } else {
            att_arr.push(LcsfValidAtt {
                payload: LcsfValidAttPayload::Data(Vec::new()),
    ...
    if let CmdPayload::Cc4Payload(payload) = &cmd_payload {
        ...
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::SubattArr(vec![
                ...
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(
                        payload.ca1_payload.sa1.to_le_bytes().to_vec(),
                    ),
                },
                // Fill data of sub-attribute SA2
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(
                        payload.ca1_payload.sa2.to_le_bytes().to_vec(),
                    ),
                },
                // Fill data of optional sub-attribute SA3
                LcsfValidAtt {
        ...
        // Fill data of optional attribute CA2
        att_arr.push(LcsfValidAtt {
            payload: if payload.is_ca2_here {
                LcsfValidAttPayload::SubattArr(vec![
                    // Fill data of optional sub-attribute SA1
                    LcsfValidAtt {
                        payload: if payload.ca2_payload.is_sa1_here {
                            LcsfValidAttPayload::Data(
                                payload.ca2_payload.sa1.to_le_bytes().to_vec(),
                            )
                        } else {
                            LcsfValidAttPayload::Data(Vec::new())
                        },
                        ...
                        payload: LcsfValidAttPayload::SubattArr(vec![
                            ...
                            LcsfValidAtt {
                                payload: if payload.ca2_payload.ca3_payload.is_sa1_here {
                                    LcsfValidAttPayload::Data(
                                        payload.ca2_payload.ca3_payload.sa1.to_le_bytes().to_vec(),
                                    )
                                } else {
                                    LcsfValidAttPayload::Data(Vec::new())
                                },
                            ...
                            LcsfValidAtt {
                                payload: LcsfValidAttPayload::SubattArr(vec![
                                    ...
                                    LcsfValidAtt {
                                        payload: LcsfValidAttPayload::Data(
                                            payload.ca2_payload.ca3_payload.ca4_payload.sa4.clone(),
                                        ),
                                    },
                                ]),
                            },
                ...
                LcsfValidAttPayload::SubattArr(Vec::new())
            },
        });
    }
    att_arr
    ...
    if let CmdPayload::Cc6Payload(payload) = &cmd_payload {
        ...
        att_arr.push(LcsfValidAtt {
            payload: LcsfValidAttPayload::SubattArr(vec![
                ...
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(
                        payload.ca9_payload.sa1.to_le_bytes().to_vec(),
                    ),
                },
                // Fill data of sub-attribute SA2
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(
                        payload.ca9_payload.sa2.to_le_bytes().to_vec(),
                    ),
                },
                // Fill data of optional sub-attribute SA3
                LcsfValidAtt {
        ...
        // Fill data of optional attribute CA10
        att_arr.push(LcsfValidAtt {
            payload: if payload.is_ca10_here {
                LcsfValidAttPayload::SubattArr(vec![
                    // Fill data of optional sub-attribute SA1
                    LcsfValidAtt {
                        payload: if payload.ca10_payload.is_sa1_here {
                            LcsfValidAttPayload::Data(
                                payload.ca10_payload.sa1.to_le_bytes().to_vec(),
                            )
                        } else {
                            LcsfValidAttPayload::Data(Vec::new())
                        },
...
mod tests {
    ...
    fn test_receive_cmd() {
        ...
        let valid_cc5_cmd = LcsfValidCmd {
            cmd_id: CMD_ID_CC5,
            att_arr: vec![
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![0xff, 0x00]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::SubattArr(vec![
                        LcsfValidAtt {
    ...
    fn test_send_cmd() {
        ...
        let valid_cc1_cmd = LcsfValidCmd {
            ...
            att_arr: vec![
                ...
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![0xd1, 0x07]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![0xa1, 0x86, 0x01, 0x00]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![6, 5, 4, 3, 2]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![0x43, 0x70, 0x63]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![4]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(Vec::new()),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![0xf0, 0x49, 0x02, 0x00]),
                },
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![2, 3, 4, 5, 6]),
                },
                LcsfValidAtt {
        ...
        let valid_cc4_cmd = LcsfValidCmd {
            cmd_id: CMD_ID_CC4,
            att_arr: vec![
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::Data(vec![0]),
                ...
                LcsfValidAtt {
                    payload: LcsfValidAttPayload::SubattArr(vec![
                        LcsfValidAtt {
                            payload: LcsfValidAttPayload::Data(vec![2]),
                        },
                        LcsfValidAtt {
                            payload: LcsfValidAttPayload::Data(vec![0xd1, 0x07]),
                        },
